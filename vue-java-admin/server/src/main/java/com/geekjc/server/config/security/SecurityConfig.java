package com.geekjc.server.config.security;

import com.geekjc.server.config.MyProperties;
import com.geekjc.server.config.security.filter.AdminAuthenticationProcessingFilter;
import com.geekjc.server.config.security.filter.MyAuthenticationFilter;
import com.geekjc.server.config.security.login.AdminAuthenticationEntryPoint;
import com.geekjc.server.config.security.url.UrlAccessDecisionManager;
import com.geekjc.server.config.security.url.UrlAccessDeniedHandler;
import com.geekjc.server.config.security.url.UrlFilterInvocationSecurityMetadataSource;
...
/**
 * @author ll
 * @date 2020年02月11日 12:00 PM
 *
 * Security 核心配置类
 */
@Configuration
@EnableWebSecurity
@EnableGlobalMethodSecurity(prePostEnabled = true) // 开启基于方法的安全认证机制，也就是说在web层的controller启用注解机制的安全确认, 只有加了@EnableGlobalMethodSecurity(prePostEnabled=true) 那么在controller层使用的 @PreAuthorize(“hasAuthority(‘admin’)”)才会生效
