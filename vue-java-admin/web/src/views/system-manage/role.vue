        ...
        >
          ...
          <el-table-column label="是否已关联系统用户" align="center">
            <template slot-scope="scope">
              <el-tag
                :type="scope.row.isRelatedSysUser == 1 ? 'success' : 'danger'"
                hit
              >{{ scope.row.isRelatedSysUser == 1 ? '&emsp;是&emsp;' : '&emsp;否&emsp;' }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="是否已关联系统资源" align="center">
            <template slot-scope="scope">
              <el-tag
                :type="scope.row.isRelatedSysMenu == 1 ? 'success' : 'danger'"
                hit
              >{{ scope.row.isRelatedSysMenu == 1 ? '&emsp;是&emsp;' : '&emsp;否&emsp;' }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="是否已关联微信账号" align="center">
            <template slot-scope="scope">
              <el-tag
                :type="scope.row.isRelatedWxAccount == 1 ? 'success' : 'danger'"
                hit
              >{{ scope.row.isRelatedWxAccount == 1 ? '&emsp;是&emsp;' : '&emsp;否&emsp;' }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column
            v-if="this.global_checkBtnPermission(['sys:role:edit','roleSetting','sys:role:delete'])"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      if (
        row.isRelatedSysUser == "1" ||
        row.isRelatedSysMenu == "1" ||
        row.isRelatedWxAccount == "1"
      ) {
